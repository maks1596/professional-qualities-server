...
namespace ProfessionalQualitiesServer.Entities.Statistics
{
    internal static class Math
    {
        ...
        public static double D(IEnumerable<int> values) => Variance(values);
        public static double Variance(IEnumerable<int> values) => Variance(values, M(values));

        public static double D(IEnumerable<int> values, double m) => Variance(values, m);
        public static double Variance(IEnumerable<int> values, double expectedValue)
        {
            return M(values.Select(x => System.Math.Pow(x - expectedValue, 2)));
        }

        public static IEnumerable<int> Multiply(IEnumerable<int> x, IEnumerable<int> y)
        {
            var count = x.Count();
            if (count != y.Count())
            {
